...
void cata_control() {
  ...
  while (true) {
    if (cata_rot.get_position() < 1000 &&
        cata.state != 1) // checks if the catapult should be lowered
    {
      cata.state = 0;
    }
    // checks if the catapult is in override mode
    if (cata.state == 0) // starts lowering the catapult if the cata is unloaded
...
void terminal_print() {
  ...
  while (true) {
    ...
    std::string voltage_str = std::to_string(
        voltage_float); // converts the float variables to strings
